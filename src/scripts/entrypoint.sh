...
proxy_config(){
  ...
  if [[ -n "${TOR_PROXY_PORT}" ]]; then
    sed -i "/SocksPort 0/c\SocksPort 0.0.0.0:${TOR_PROXY_PORT}" $TOR_CONFIG_FILE
    echo "Updated proxy binding and port..."
    echo "[WARN] Even we get a warning from TOR service (look below logs), we do not need to worry. SocksPort is exposed to the localhost..."
    echo "[WARN] ... but is inside of the docker network. The host machine cannot reach that service because that port is closed if we want..."
    echo "[WARN] ... to reach outside of docker network. In that case, the host machine"
  fi
...
copy_files(){
  ...
  cp /tmp/tor/tor-man.txt ${DATA_DIR}
